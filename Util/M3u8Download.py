...
class M3u8DownloadBlock:
    @abstractmethod
    def m3u8Block(self, result: bool):
        pass
...
class M3u8Download:
    ...
    def __init__(self, url, name, path="", block: M3u8DownloadBlock = None, max_workers=64, num_retries=5):
        self.url = url
        self.name = name
        self.path = path
        ...
        if self.success_sum == self.ts_sum:
            ...
            if block is not None:
                block.m3u8Block(True)
        elif block is not None:
            block.m3u8Block(False)
    ...
    def output_mp4(self):
        ...
        if len(self.path) > 0:
            os.system(f"mv {self.name}.mp4 {self.path}")
        # os.system(f'rm -rf ./{self.name} ./{self.name}.m3u8')
        print(f"Download successfully --> {self.path,self.name}")


if __name__ == "__main__":
    url = 'https://www.zhuticlub.com:65/20190322/pktnp7b2/2000kb/hls/index.m3u8'
    name = 'abc'
    M3u8Download(url, name)
